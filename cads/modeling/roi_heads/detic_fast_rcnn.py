...
class DeticFastRCNNOutputLayers(FastRCNNOutputLayers):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.use_fed_loss or self.ignore_zero_cats:
            ...
            self.register_buffer('freq_weight', freq_weight.cpu())
        else:
    ...
    def inference(self, predictions, proposals, mlr_scores=None):
        """
        enable use proposal boxes
        """
        predictions = (predictions[0], predictions[1])
        boxes = self.predict_boxes(predictions, proposals)
        scores = self.predict_probs(predictions, proposals, mlr_scores)
    ...
    def predict_probs(self, predictions, proposals, mlr_scores=None):
        """
        support sigmoid
        """
        ...
        if mlr_scores != None:
            ...
            mlr_scores = torch.cat([mlr_scores, mlr_scores.new_zeros([1,1])], dim = 1)
            mlr_scores_list = []
            for idx, mlr_scores_per_img in enumerate(mlr_scores):
                mlr_scores_per_img = mlr_scores_per_img.unsqueeze(0).repeat((num_inst_per_image[idx], 1))
                mlr_scores_list.append(mlr_scores_per_img)

            mlr_scores = torch.cat(mlr_scores_list, dim = 0)
            gamma = self.cads_gamma
            probs = (probs**(1 - gamma)) * (mlr_scores ** gamma) 
