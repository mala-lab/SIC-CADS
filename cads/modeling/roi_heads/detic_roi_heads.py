...
class DeticCascadeROIHeads(CascadeROIHeads):
    ...
    def _forward_box(self, features, proposals, targets=None, 
        ...
        if self.training:
            ...
            if pred_mlr_scores != None:
                pred_mlr_scores = torch.cat([pred_mlr_scores, pred_mlr_scores.new_zeros([pred_mlr_scores.shape[0], 1])], dim=1)
                pred_mlr_scores = pred_mlr_scores.unsqueeze(dim=1).repeat(1, scores[0].shape[0],1)
                gamma = self.cads_gamma
                for idx, (score, pred_mlr_score) in enumerate(zip(scores, pred_mlr_scores)):
                    score = score**(1 - gamma) * pred_mlr_score**(gamma)
                    scores[idx] = score
    ...
    def forward(self, images, features, proposals, targets=None,
        ...
        if self.training:
            ...
            pred_instances = self._forward_box(
                features, proposals, classifier_info=classifier_info, pred_mlr_scores=pred_mlr_scores)
