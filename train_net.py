...
from cads.data.custom_build_augmentation import build_custom_augmentation
from cads.data.custom_dataset_dataloader import  build_custom_train_loader
from cads.data.custom_dataset_mapper import CustomDatasetMapper, DetrDatasetMapper
from cads.custom_solver import build_custom_optimizer
from cads.evaluation.oideval import OIDEvaluator
from cads.evaluation.custom_coco_eval import CustomCOCOEvaluator
from cads.modeling.utils import reset_cls_test
from cads.evaluation.mlr_eval import mlr_evaluation
from cads.modeling.meta_arch.ov_mlr import OvMlr

logger = logging.getLogger("detectron2")

def do_test_mlr(cfg, model):
    ...
    for d, dataset_name in enumerate(cfg.DATASETS.TEST):
        ...
        mlr_evaluation(model, data_loader, cfg.MLR.DATASET)
...
def setup(args):
    ...
    setup_logger(output=cfg.OUTPUT_DIR, \
        distributed_rank=comm.get_rank(), name="cads")
    return cfg
...
def main(args):
    ...
    if args.inference_mlr:
        DetectionCheckpointer(model, save_dir=cfg.OUTPUT_DIR).resume_or_load(
            cfg.MODEL.WEIGHTS, resume=args.resume
        )
        do_test_mlr(cfg, model)
        return
...
if __name__ == "__main__":
    ...
    parser.add_argument("--inference-mlr", action="store_true")
